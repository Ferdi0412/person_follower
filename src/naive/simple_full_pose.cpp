...
#include "naive/SimpleHandsWrapper.hpp"
...
ros::Publisher hand_annot;
ros::Publisher hand_vis;
...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,
    ...
    // Hands
    cv_bridge::CvImagePtr hand_frame;
    if ( hand_annot.getNumSubscribers() ) {
        hand_frame = boost::make_shared<cv_bridge::CvImage>(
            rgb->header,
            rgb->encoding,
            rgb->image.clone()
        );
    ...
    PoseArrayMsg hand_msg;
    hands.process(rgb, depth, pose_msg, intrinsics, hand_msg, hand_frame);

    hand_pub.publish(hand_msg);

    if ( hand_frame )
        hand_annot.publish(hand_frame->toImageMsg());

    if ( hand_vis )
        hand_vis.publish(hands.pose_visualization(hand_msg));
}
...
int main(int argc, char** argv) {
    ...
    hand_pub     = nh.advertise<PoseArrayMsg>("/person_follower/hands", 1);
    hand_annot   = nh.advertise<sensor_msgs::Image>("/person_follower/hands/image_raw", 1);
    hand_vis     = nh.advertise<visualization_msgs::MarkerArray>("/person_follower/hands/markers", 1);

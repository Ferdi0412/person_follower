#!/usr/bin/env python3
"""Implements the control node logic.

Uses a finite state machine to implement "latching" logic.
Publishes a goal position to face person, or within 1 meter of person.
"""
...
class StateDefinition:
    ...
    continue_on: List[str]     = field(default_factory = list)
    ...
    @property
    def hold_gestures(self) -> int:
        return {self.enter, *self.continue_on}
...
class StateMachine:
    ...
    def update(self, gesture):
        ...
        if active is not None and active.continue_on:
            if gesture in active.hold_gestures:
                return self._current_state
            # self._current_state = self._idle_state
            # return self._current_state
    ...
    def from_json(filename, idle = 'IDLE', debounce = 3):
        ...
        if isinstance(data, dict):
            definitions = [StateDefinition(
                ...
                enter       = value['enter'],
                exit        = value.get('exit'),
                continue_on = value.get('continue_on', [])
...
if __name__ == "__main__":
    ...
    node = ControlNode(StateMachine.from_json(package_root + "/scripts/states.json"))
    rospy.spin()